...
def main():
    ...
    print("Year range:", capital["year"].min(), "-", capital["year"].max())

    print("\nCompanies per year:")

    print(capital.groupby("year")["company_id"].nunique().to_string())
    ...
    latest = capital[capital["year"] == latest_year]

    print(f"\n=== {latest_year} PATTERN DISTRIBUTION ===")

    distribution = latest["pattern_label"].value_counts()
    ...
    capital["previous_year"] = capital.groupby("company_id")["year"].shift(1)

    changes = capital[
        capital["previous_pattern"].notna()
        & (capital["pattern_label"] != capital["previous_pattern"])
    ].copy()

    changes = changes[
        ["company_id", "previous_year", "year", "previous_pattern", "pattern_label"]
    ]
    ...
    intelligence = pd.read_excel(CASHFLOW_FILE)

    latest_allocation = latest[["company_id", "pattern_label"]].copy()
    ...
    intelligence = intelligence.merge(latest_allocation, on="company_id", how="left")

    intelligence.to_excel(CASHFLOW_FILE, index=False)
    ...
    print("Cash-flow intelligence rows:", len(intelligence))

    print("Unique companies:", intelligence["company_id"].nunique())

    print(
        "Companies without allocation:",
        intelligence["capital_allocation_label"].isna().sum(),
    )
