...
def create_kpi_table(
    ...
    for title, value, suffix in metrics:
        ...
        cell = Paragraph(
            f"<b>{title}</b><br/><font size=14>{display}</font>",
            body_style
        )
...
def build_tearsheet(
    ...
    if not sector_data.empty:
        ...
        broad_sector = sector_data.iloc[0][
            "broad_sector"
        ]
