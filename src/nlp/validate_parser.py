...
def main():
    ...
    parsed_5yr = parsed[parsed["period_years"] == 5].copy()
    ...
    ratios = ratios.sort_values("company_id").groupby("company_id").last().reset_index()
    ...
    for _, row in parsed_5yr.iterrows():
        ...
        match = ratios[ratios["company_id"] == company]
    ...
    result.to_csv("output/parser_cross_validation.csv", index=False)

    print("Cross-validation rows:", len(result))

    print(
        "Manual review:",
        (result["flag"] == "MANUAL REVIEW").sum() if not result.empty else 0,
    )

    print("Saved: output/parser_cross_validation.csv")
